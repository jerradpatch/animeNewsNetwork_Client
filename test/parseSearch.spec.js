...
var EncyclopediaSearchNamePage_1 = require("./RawPages/EncyclopediaSearchNamePage");
var EncyclopediaAnimePage_1 = require("./RawPages/EncyclopediaAnimePage");
var EncyclopediaAnime_EpisodesPage_1 = require("./RawPages/EncyclopediaAnime_EpisodesPage");
var EncyclopediaMangaPage_1 = require("./RawPages/EncyclopediaMangaPage");
...
describe('Testing the ANN Parse SearchPage client', function () {
    ...
    describe('Test encyclopedia Page Objects parse correct information', function () {
        it('test encyclopedia/search/name?only=anime&q=jinki saved page is parsed correctly', function (done) {
            var spMod = new EncyclopediaSearchName_1.EncyclopediaSearchName(EncyclopediaSearchNamePage_1.encyclopediaSearchNamePage);
            testSearchPage(spMod);
            done();
        });
        it('test encyclopedia/search/name?only=anime&q=jinki test api returns expected values', function (done) {
            var spMod = new EncyclopediaSearchName_1.EncyclopediaAnime(EncyclopediaAnimePage_1.encyclopediaAnimePage);
            testAnimePage(spMod);
            done();
        });
        it('test https://www.animenewsnetwork.com/encyclopedia/manga.php?id=11608 test api returns expected values', function (done) {
            var spMod = new EncyclopediaSearchName_1.EncyclopediaAnime(EncyclopediaMangaPage_1.encyclopediaMangaPage);
            testMangaPage(spMod);
            done();
        });
        it('test https://www.animenewsnetwork.com/encyclopedia/anime.php?id=4658&page=25 test api returns expected values', function (done) {
            var spMod = new EncyclopediaSearchName_1.EncyclopediaAnimeEpisodes(EncyclopediaAnime_EpisodesPage_1.encyclopediaAnime_episodesPage);
    ...
    describe('Test encyclopedia Page Objects expected pare matches actual parse', function () {
        it('test encyclopedia/search/name?only=anime&q=jinki search page is as expected', function (done) {
            rp(baseUrl + '/encyclopedia/search/name?only=anime&q=jinki')
                .then(function (page) {
                var spMod = new EncyclopediaSearchName_1.EncyclopediaSearchName(page);
                testSearchPage(spMod);
                done();
            });
        });
        it('test /encyclopedia/manga.php?id=11608 manga page is as expected', function (done) {
            rp(baseUrl + '/encyclopedia/manga.php?id=11608')
                .then(function (page) {
                var spMod = new EncyclopediaSearchName_1.EncyclopediaAnime(page);
                testMangaPage(spMod);
                done();
            });
        });
        ...
        it('test /encyclopedia/anime.php?id=4658&page=25 episode page is as expected', function (done) {
            rp(baseUrl + '/encyclopedia/anime.php?id=4658&page=25')
                .then(function (page) {
                var spMod = new EncyclopediaSearchName_1.EncyclopediaAnimeEpisodes(page);
                testEpisodesPage(spMod);
                done();
            })
                .catch(function (e) {
    ...
    describe('Test library client results with search page parsing', function () {
        ...
        var ann = new index_1.ANN_Client(ops);
        it('it should have parsed search result for single title', function (done) {
            var title = 'ulysses jehanne darc to renkin no kishi';
            ann['parseSearchPage'](title).then(function (parse) {
        ...
        it('it should have parsed search result for single title Zoids', function (done) {
            var titleZ = 'Zoids Wild Raw';
            ann['parseSearchPage'](titleZ).then(function (parse) {
        ...
        it('it should have parsed search results for multi titles', function (done) {
            var titles = ['ulysses jehanne darc to renkin no kishi'];
            ann['parseSearchPageTitles'](titles).then(function (parse) {
                expect(parse.anime.length).to.be.equal(1);
                expect(parse.manga.length).to.be.equal(0);
                done();
            })
        ...
        it('it should have parse search results matching nonParse search results', function (done) {
            var titles = ['ulysses jehanne darc to renkin no kishi'];
            ann.findTitlesLike(titles).then(function (parse) {
                expect(parse.anime.length).to.be.equal(1);
                expect(parse.manga.length).to.be.equal(0);
                done();
            }).catch(function (e) {
        ...
        it('it should return more results than a regular search', function (done) {
            var ops2 = { apiBackOff: 10 };
            var ann2 = new index_1.ANN_Client(ops2);
            var titles = ['ulysses jehanne darc to renkin no kishi'];
            Promise.all([
                ann2.findTitlesLike(titles),
                ann.findTitlesLike(titles)
            ]).then(function (_a) {
                var nonParse = _a[0], parse = _a[1];
    ...
    describe('Test that there is an improvement in results', function () {
        ...
        it('It should have a larger number of results per title given a list of titles', function (done) {
            ...
            Promise.all(titles.map(function (title) {
                return Promise.all([ann2.findTitlesLike([title]),
                    ann.findTitlesLike([title])]).then(function (_a) {
                    var nonParse = _a[0], parse = _a[1];
                    var anime = (((parse.anime && parse.anime.length || 0) - (nonParse.anime && nonParse.anime.length || 0)) / (nonParse.anime && nonParse.anime.length || 1)) * 100;
                    var manga = (((parse.manga && parse.manga.length || 0) - (nonParse.manga && nonParse.manga.length || 0)) / (nonParse.manga && nonParse.manga.length || 1)) * 100;
                    return { anime: anime, manga: manga };
                });
            })).then(function (res) {
                var sumImpAni = res.reduce(function (acc, c) { return acc + c.anime; }, 0);
                console.log("Avg improvement anime: ", sumImpAni / res.length + '%');
                expect(sumImpAni / res.length).to.be.gt(0);
                var sumImpMan = res.reduce(function (acc, c) { return acc + c.manga; }, 0);
        ...
        it('Show the increase in request completion time due parsing the search page', function (done) {
            this.timeout(10 * 60 * 60 * 1000);
            var title = 'Cardcaptor Sakura Clear Card Hen';
            var ops = {
                apiBackOff: 4,
                useDerivedValues: true,
                parseSearchPage: true
            ...
            Promise.all([
                ann.findTitlesLike([title]).then(function () {
                    return Date.now();
                }),
                ann2.findTitlesLike([title]).then(function () {
                    return Date.now();
                })
            ]).then(function (_a) {
                var parseEnd = _a[0], nonParseEnd = _a[1];
                console.log('Time increase due to parsing the page', "nonParse Duration:", nonParseEnd - startTime.getMilliseconds(), "parse Duration:", parseEnd - startTime.getMilliseconds(), "increased Duration Sec:", (parseEnd - nonParseEnd) / 1000);
...
//# sourceMappingURL=parseSearch.spec.js.map